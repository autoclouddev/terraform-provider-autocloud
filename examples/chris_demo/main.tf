...
data "autocloud_blueprint_config" "cloudfront_processor" {
  ...
  variable {
    ...
    # value   = "Site content for test static site {{namespace}}-{{environment}}-{{name}}"
    value = format("Site content for test static site content")
  }
  ###
  # Override environment type for conditional usage
  variable {
    name = "environment"
    type = "radio"
  }
...
data "autocloud_blueprint_config" "route53_processor" {
  ...
  variable {
    ...
    value = "" # Set to collected domain name
  }
  ###
  # Set records
  variable {
...
data "autocloud_blueprint_config" "final" {
  ...
  variable {
    ...
    helper_text  = "Domain name where static site content will be served from"
    type         = "shortText"
    # conditional {
    #   source    = "cloudfront.environment"
    #   condition = "nonprod"
    #   content {
    #       suffix = ".nonprod.unstyl.com"
    #   }
    # }
    # conditional {
    #   source    = "cloudfront.environment"
